...
function ImageList() {
    ...
    const handleImportImage = (event) => {
        ...
        fetch(`${process.env.REACT_APP_iThink}/api/images`, {
            method: "POST",
            body: formData,
        })
            .then((response) => response.json())
            .then((data) => {
                setImages([
                    ...images,
                    { _id: data.image_id, filename: file.name },
                ]);
            })
            .catch((error) => console.error(error));
    };
    ...
    const deleteImage = (imageId) => {
        fetch(`${process.env.REACT_APP_iThink}/api/images/${imageId}`, {
            method: "DELETE",
        })
            .then(() => {
            ...
            <div className="grid grid-cols-3 gap-4 p-4">
                {images.map((image) => (
                    <div key={image._id} className="relative">
                        <img
                            src={`${process.env.REACT_APP_iThink}/api/images/${image.filename}`}
                            alt={image.filename}
